 ...
 * @package wascore
 * @version $Id: mysql.class.php,v 1.1 2011/02/01 13:00:52 pfokker Exp $
 */
 ...
 */
class DatabaseMysql {
    /** @var string database type, e.g. 'mysql' */
    var $db_type;

    /** @var string database server, e.g. 'localhost' or 'db.example.com:3306' */
    var $db_server;

    /** @var string part of credentials for database access */
    var $db_username;

    /** @var string part of credentials for database access */
    var $db_password;

    /** @var string database to use, e.g. 'was' */
    var $db_name;

    /** @var string table name prefix, e.g. 'was_' */
    var $prefix;

    /** @var resource database link identifier */
    var $db_link;

    /** @var integer the number of queries executed sofar */
    var $query_counter;
    ...
    /** @var bool if true switch debugging on */
    var $debug;
     ...
     *
     * @param string database server, e.g. 'localhost' or 'db.example.com:3306'
     * @param string part of credentials
     * @param string part of credentials
     * @param string database to use, e.g. 'was'
    ...
    function escape($unescaped_string) {
        $retval = mysql_real_escape_string($unescaped_string,$this->db_link);
        $this->errno = mysql_errno();
        $this->error = mysql_error();
        return $retval;
    } // escape()
     ...
     *
     * @param string valid SQL statement
     ...
     *
     * @param string (optional) tablename (unused in MySQL)
     * @param string (optional) fieldname (unused in MySQL)
     ...
     *
     * @param string valid SQL statement
     * @param int optional limitation of the number of records returned
     * @param int optional number of records to skip; $offset = 0 means start with the first
     ...
     *
     * @param string name of the table to drop (prefix will be added automatically)
     ...
     *
     * @param string name of the table to check (prefix will be added automatically)
     ...
     *
     * @param array a generic table definition (not database-specific)
     * @return int|bool FALSE on failure or otherwise the # of affected rows (always 0)
     ...
     *
     * Note 4: even though comments are not yet stored in the MySQL database, the corresponding
     * COMMENT-clauses are generated, if only for documentation/debugging purposes. These
     * clauses are correctly parsed by MySQL but they are subsequently discarded.
     *
     * Note 5:
    ...
    function column_definition($fielddef) {
        ...
        if (empty($field['type'])) { // special case, e.g. bool or serial
            ...
            if ($field['default']) {
                if (isset($fielddef['default'])) {
                    if (strcasecmp($fielddef['default'],'null') == 0) { // see the note above
    ...
    function dump(&$data,$drop=TRUE,$tables='') {
        ...
        foreach($tables_to_dump as $table) {
            ...
            // 2D -- the following column types don't need quotes, all others do
