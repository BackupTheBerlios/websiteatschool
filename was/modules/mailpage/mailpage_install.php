 ...
 *
 * This file contains the mailpage module installer.
 ...
 * @version $Id: mailpage_install.php,v 1.1 2013/06/20 14:41:34 pfokker Exp $
 */
 ...
 */
function mailpage_install(&$messages,$module_id) {
    return TRUE;
} // mailpage_install()
 ...
 * Note that this minimalistic 'mailpage' module does not need any
 ...
 * @return bool TRUE on success + output via $messages, FALSE otherwise
 */
function mailpage_upgrade(&$messages,$module_id) {
    return TRUE;
} // mailpage_upgrade()
 ...
 * @return bool TRUE on success + output via $messages, FALSE otherwise
 */
function mailpage_uninstall(&$messages,$module_id) {
    return TRUE;
} // mailpage_uninstall()
 ...
 *
 * this routine is a no-op because there is no mailpage demodata
 ...
 * @return bool TRUE on success + output via $messages, FALSE otherwise
 */
function mailpage_demodata(&$messages,$module_id,$config,$manifest) {

/*
We need to add a record for the contact page (located in area)
which replaces the HTMLpage in the initial data (see also sitemap)

We could use the email address of Amelia Cackle as the principal
and perhaps the name of the installer as an alternative name?
