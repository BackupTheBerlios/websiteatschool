 ...
 * @version $Id: mypage_admin.php,v 1.2 2011/02/03 14:04:00 pfokker Exp $
 */
 ...
 */
function mypage_disconnect(&$output,$area_id,$node_id,$module) {
 ...
 * @return bool TRUE on success, FALSE otherwise
 */
function mypage_connect(&$output,$area_id,$node_id,$module) {
    $output->add_message('STUB: '.__FUNCTION__.'() in '.__FILE__.' ('.__LINE__.')');
    // foreach($module as $k => $v) {$output->add_message("module[$k] => $v"); }
    return TRUE;
} // mypage_connect()
 ...
 */
function mypage_show_edit(&$output,$area_id,$node_id,$module,$viewonly,$edit_again,$href) {
 ...
 * @return bool TRUE on success + output stored via $output, FALSE otherwise
 */
function mypage_save(&$output,$area_id,$node_id,$module,$viewonly,&$edit_again) {
